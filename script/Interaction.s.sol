...
contract FundSubscription is Script {
    ...
    function fundSubscription(address vrfCoordinator, uint64 subId, address link, uint256 deployerKey) public {
        ...
        if (block.chainid == 31337) {
            ...
            vm.startBroadcast(deployerKey);
            LinkToken(link).transferAndCall(vrfCoordinator, FUND_AMOUNT, abi.encode(subId)); //come back to learn abi.encode
            vm.stopBroadcast();
        }
    }
...
contract AddConsumer is Script {
    function addConsumer(address raffle, address vrfCoordinator, uint64 subId, uint256 deployerKey) public {
        ...
        console.log("Using vrfCoordinator ", vrfCoordinator);
        console.log("On ChainId", block.chainid);
        vm.startBroadcast(deployerKey); //when starting the broadcast we can pass in the private key based off the evm chain
